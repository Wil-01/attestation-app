...
// Imports de React et des dépendances
...
// Imports de nos modules et schémas
...
// On importe dynamiquement le composant qui utilise les API web pour le PDF
const DynamicPdfActions = dynamic(
  () => import('./PdfActions').then((mod) => mod.PdfActions),
  { 
    ssr: false, // On s'assure qu'il n'est jamais rendu côté serveur
    loading: () => <Button className="w-full" size="lg" disabled>Génération du PDF...</Button> 
  }
);

// Composant réutilisable pour les champs de date
...
export function AttestationForm() {
  ...
  function onSubmit(values: TAttestationSchema) {
    console.log("Données du formulaire validées, préparation du PDF...", values);
    setFormData(values);
  }
    ...
    <div className="border p-4 sm:p-6 md:p-8 rounded-lg shadow-md bg-white">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <FormField name="hostGender" render={({ field }) => (
            <FormItem className="space-y-2"><FormControl>
                <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex items-center space-x-4">
                  <FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="Madame" /></FormControl><FormLabel className="font-normal">Madame</FormLabel></FormItem>
                  <FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="Monsieur" /></FormControl><FormLabel className="font-normal">Monsieur</FormLabel></FormItem>
                </RadioGroup>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField name="hostLastName" render={({ field }) => (
              <FormItem><FormLabel>Nom :</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
            <FormField name="hostFirstName" render={({ field }) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            ...
            <FormField name="hostBirthPlace" render={({ field }) => (
              <FormItem><FormLabel>à </FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
          </div>

          <FormField name="hostAddress" render={({ field }) => (
            <FormItem><FormLabel>Demeurant au</FormLabel><FormControl><Input placeholder="Adresse ligne 1" {...field} /></FormControl><FormMessage /></FormItem>
          )}/>
          <FormField name="hostAddressLine2" render={({ field }) => (
            <FormItem><FormControl><Input placeholder="Adresse ligne 2 (optionnel)" {...field} /></FormControl><FormMessage /></FormItem>
          )}/>
          
          <p className="pt-4 text-gray-600">Certifie sur l'honneur héberger à mon domicile ci-dessus mentionné :</p>
          
          <FormField name="hostedGender" render={({ field }) => (
            <FormItem className="space-y-2"><FormControl>
                <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex items-center space-x-4">
                  <FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="Madame" /></FormControl><FormLabel className="font-normal">Madame</FormLabel></FormItem>
                  <FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="Monsieur" /></FormControl><FormLabel className="font-normal">Monsieur</FormLabel></FormItem>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField name="hostedLastName" render={({ field }) => (
              <FormItem><FormLabel>Nom :</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
            <FormField name="hostedFirstName" render={({ field }) => (
              <FormItem><FormLabel>Prénom :</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            ...
            <FormField name="hostedBirthPlace" render={({ field }) => (
              <FormItem><FormLabel>à :</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start pt-6">
             <FormField name="attestationPlace" render={({ field }) => (
              <FormItem><FormLabel>Fait à :</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
          ...
          <div>
            ...
            <div className="mt-2">
              {signatureValue ? (
                <div className="border rounded-md p-2 h-24 w-48 relative bg-gray-50">
                  <Image src={signatureValue} alt="Signature de l'hébergeur" layout="fill" objectFit="contain" />
                </div>
              ) : null}
              <Button
                type="button"
          ...
          <div className="border-t pt-8 mt-8">
            {!formData ? (
              <Button type="submit" className="w-full" size="lg">
                Générer l'aperçu du PDF
              </Button>
            ) : (
              <DynamicPdfActions formData={formData} />
            )}
          </div>
        </form>
      </Form>
